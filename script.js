...
let processList=[]; 
const 
colors=['#4f46e5','#10b981','#8b5cf6','#f59e0b','#ec4899','#
...
const priorityg = 
document.getElementById('priorityDirectionGroup'); 
const timeQuantum = 
document.getElementById('timeQuantum'); 
document.getElementById('addProcess').addEventListener(
 'click', addProcess); 
document.getElementById('calculate').addEventListener('cl
 ick', calculate); 
...
const burstTime = 
document.getElementById('burstTime'); 
const priority = document.getElementById('priority'); 
if (!processID.value || !arrivalTime.value || 
!burstTime.value) { 
alert('Please fill in Process ID, Arrival Time, and 
Burst Time'); 
return; 
} 
if (algorithmSelect.value === 'Priority' && 
!priority.value) { 
alert('Please enter a priority value for Priority 
Scheduling'); 
return; 
} 
const process = { 
...
priority: algorithmSelect.value === 'Priority' ? 
parseInt(priority.value) : 0 
}; 
processList.push(process); 
updateProcessTable(); 
clearInputs([processID, arrivalTime, burstTime, 
priority]); 
} 
...
document.getElementById('avgTurnaroundTime').textCont
 ent = '0.00'; 
...
const processes = [...processList].sort((a, b) => 
...
ganttData.push({ processID: process.processID, 
startTime, endTime: currentTime }); 
}); 
return { completedProcesses, ganttData, maxTime: 
currentTime }; 
} 
...
currentTime = Math.min(...processes.map(p => 
p.arrivalTime)); 
continue; 
} 
const shortestJob = availableProcesses.reduce((prev, 
curr) =>  
...
shortestJob.arrivalTime - shortestJob.burstTime 
}); 
ganttData.push({ processID: shortestJob.processID, 
startTime, endTime: currentTime }); 
const index = processes.findIndex(p => p.processID 
=== shortestJob.processID); 
...
const executeTime = Math.min(quantum, 
...
completionTime: currentTime, 
turnaroundTime: currentTime - 
highestPriorityJob.arrivalTime, 
waitingTime: currentTime - 
highestPriorityJob.arrivalTime - 
highestPriorityJob.burstTime 
}); 
ganttData.push({ processID: 
highestPriorityJob.processID, startTime, endTime: 
currentTime }); 
const index = processes.findIndex(p => p.processID 
=== highestPriorityJob.processID); 
processes.splice(index, 1); 
} 
return { completedProcesses, ganttData, maxTime: 
currentTime }; 
} 
function updateResults({ completedProcesses, ganttData, 
maxTime }) { 
...
completedProcesses.reduce((sum, p) => sum + 
p.turnaroundTime, 0) / completedProcesses.length; 
const avgWaitingTime = 
completedProcesses.reduce((sum, p) => sum + 
p.waitingTime, 0) / completedProcesses.length; 
document.getElementById('avgTurnaroundTime').textCont
 ent = avgTurnaroundTime.toFixed(2); 
document.getElementById('avgWaitingTime').textContent 
...
colors[process.processID % colors.length]; 
bar.style.left = `${(process.startTime / maxTime) * 
100}%`; 
bar.style.width = `${((process.endTime - 
process.startTime) / maxTime) * 100}%`; 
